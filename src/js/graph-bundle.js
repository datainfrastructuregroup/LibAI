...
// src/js/graph-visualization.js
function renderForceGraph(container, data, opts = {}) {
  ...
  const svg = select_default2(wrapper).append("svg").attr("viewBox", [0, 0, width, height]).style("width", "100%").style("height", `${height}px`).style("cursor", "grab");
  const g = svg.append("g");
  const zoomBehavior = zoom_default2().scaleExtent([0.2, 4]).on("zoom", (e) => g.attr("transform", e.transform));
  ...
  if (Array.isArray(nodesIn)) {
    ...
    nodesArr = Object.entries(nodesIn).map(([id2, v]) => {
      const layout = v?.meta?.layout;
      ...
      const displayLabel = title || summary || id2;
      const description = summary && summary !== title ? summary : v?.meta?.description;
  ...
  const link = g.append("g").selectAll("line").data(links).join("line").attr("stroke", colorLink).attr("stroke-opacity", 0.5).attr("stroke-width", 1.5);
  const node = g.append("g").selectAll("circle").data(nodes).join("circle").attr("r", (d) => d.isPerson ? 12 : 8).attr("fill", (d) => d.isPerson ? colorAccent : colorCardBg).attr("stroke", (d) => d.isPerson ? colorAccent : colorLink).attr("stroke-width", 2).style("cursor", "pointer");
  const label = g.append("g").selectAll("text").data(nodes).join("text").text((d) => d.label).attr("font-size", 13).attr("dx", 14).attr("dy", 4).attr("fill", colorText).attr("opacity", 0.9).style("user-select", "none").style("pointer-events", "none");
...
${d.description}` : d.label);
  ...
  const simulation = simulation_default(nodes).force(
    "link",
    link_default(links).id((d) => d.id).distance(120).strength(0.5)
    // how strongly links pull nodes together
  ).force("charge", manyBody_default().strength(-120)).force("center", center_default(width / 2, height / 2)).force("collision", collide_default(30)).force(
    "orphan-radial",
    // Pull orphan nodes toward a ring near the center.
    // Connected nodes get strength 0 so this doesn't affect them.
    radial_default(80, width / 2, height / 2).strength((n) => connectedIds.has(n.id) ? 0 : 0.8)
  ).alphaDecay(0.02).alphaMin(1e-3).on("tick", ticked);
  node.call(
  ...
  resetBtn.addEventListener("click", () => {
    svg.transition().duration(500).call(zoomBehavior.transform, identity2);
