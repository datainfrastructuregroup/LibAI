# Liberation + AI + 11ty
A rebuild of the LibAI site made with 11ty, and driven by markdown files - an accessible technology for easy updating by anyone who is comfortable using Github.
...
This site is configured for automatic deployment to GitHub Pages. The deployment workflow will:

1. Trigger on pushes to the `main` branch
2. Build the site using Eleventy
3. Deploy the `_site` directory to GitHub Pages

### Manual Deployment

If you need to deploy manually:

1. Update the repository name in the build script:
   ```bash
   # Replace [REPOSITORY_NAME] with your actual repository name
   sed -i 's/\[REPOSITORY_NAME\]/your-repo-name/g' package.json
   ```
...
3. Deploy the `_site` directory to your GitHub Pages branch

### Local Development with GitHub Pages Path

To test locally with the GitHub Pages path prefix:

```bash
ELEVENTY_BASEPATH=/your-repo-name npm start
```
